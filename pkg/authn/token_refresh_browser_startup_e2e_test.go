...
// cleanup boundaries. TestE2ERefreshBrowserBootstrap uses the same launcher
// with real Chrome and the complete TLS portal journey.
func TestE2ERefreshBrowserStartup(t *testing.T) {
...
setInterval(() => {}, 1000);
`, profile)
		endpoint, stop, err := startRefreshBrowser(ctx, cmd, profile)
		if err != nil {
			t.Fatal(err)
		}
		t.Cleanup(stop)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			endpoint, stop, err := startRefreshBrowser(ctx, cmd, t.TempDir())
			if stop != nil {
				t.Cleanup(stop)
			}
	...
	t.Run("missing executable", func(t *testing.T) {
		...
		_, stop, err := startRefreshBrowser(t.Context(), cmd, profile)
		if stop != nil {
			t.Cleanup(stop)
		}
